...
export class CalendarComponent implements OnDestroy {
    ...
    combineLatest([sApi.getAll(), eApi.get(), cApi.getAll()])
      .pipe(
        finalize(() => {
          ...
          this._calendar.api.intercept('delete-event', (obj: any) => {
            eApi.delate(obj.id).subscribe();
          });

          this._calendar.api.intercept('edit-event', (obj: any) => {
            console.log(obj);
          });
        })
      )
      .subscribe(([serviceData, eventsData, clientsData]) => {
        ...
        console.log(
          this.clientList.map((user) => user.fullname).indexOf(item.user ?? '')
        );
  ...
  createSchedule(data: any) {
    ...
    const reqData: Schedule = {
      ...
      type: this.serviceList[data.service].id,
      text: data.text,
